...
async function readStream(res, adapter, onDelta) {
  ...
  // Most providers stream server-sent events; Ollama streams one JSON object per line.
  const ndjson = adapter.streamFormat === 'ndjson';
  ...
  if (ndjson) feed(buf.trim());
  else if (buf.trim().startsWith('data:')) feed(buf.trim().slice(5).trim());
  return state;
}
