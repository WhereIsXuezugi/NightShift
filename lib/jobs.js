...
import { REPEATS, validTz, serverTz } from './time.js';
...
export function parseSpec(body, { store, settings }) {
  ...
  const repeat = parseRepeat(pick(body, 'repeat') ?? pick(steps[0], 'repeat'), pick(body, 'timezone', 'tz'));
  ...
  if (repeat) {
    if (!['at', 'now'].includes(parsed[0].mode)) bad('A repeating message needs mode "at" (or "now", to start right away).');
    parsed[0].repeat = repeat;
    // The time of day it repeats at comes from the first send.
    if (parsed[0].mode === 'now') parsed[0].anchorAt = Date.now();
  }
...
/** "daily" | { every: "daily", timezone } | null  ->  { every, timezone } | null */
export function parseRepeat(value, timezone) {
  if (value == null || value === '' || value === 'none' || value === false) return null;
  const every = typeof value === 'string' ? value : value.every;
  if (!REPEATS.includes(every)) bad(`repeat must be one of: ${REPEATS.join(', ')}, or "none".`);
  const tz = (typeof value === 'object' && value.timezone) || timezone || serverTz();
  if (!validTz(tz)) bad(`Unknown timezone "${tz}". Use an IANA name such as "Europe/Berlin".`);
  return { every, timezone: tz };
}
...
export function createChain({ store, scheduler, events }, steps, source = 'web') {
  ...
  for (const step of steps) {
    const job = {
      ...
      ...step,
      chainId,
      // A Claude Code chain that starts a new session starts a new one each time it repeats.
      newSession: step.provider === 'claude-code' ? !step.sessionId : undefined,
...
export function patchJob({ store, scheduler }, job, patch) {
  ...
  const repeat = pick(patch, 'repeat');
  if (repeat !== undefined) {
    if (job.dependsOn && repeat && repeat !== 'none') bad('Only the first message of a chain can repeat; the rest repeat with it.');
    set.repeat = parseRepeat(repeat, pick(patch, 'timezone', 'tz') || job.repeat?.timezone);
  }
  ...
  const finalMode = set.mode ?? job.mode;
  const finalRepeat = 'repeat' in set ? set.repeat : job.repeat;
  if (finalRepeat && !['at', 'now'].includes(finalMode)) {
    bad('A repeating message needs mode "at".');
  }
